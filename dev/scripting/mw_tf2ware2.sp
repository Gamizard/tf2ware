...
#include tf2ware\hitenemy.inc
#include tf2ware\spycrab.inc
#include tf2ware\kamikaze.inc
#include tf2ware\math.inc
#include tf2ware\sawrun.inc
#include tf2ware\barrel.inc
#include tf2ware\needlejump.inc
#include tf2ware\hopscotch.inc
#include tf2ware\airblast.inc
#include tf2ware\movement.inc
#include tf2ware\flood.inc
#include tf2ware\simonsays.inc
#include tf2ware\bball.inc
#include tf2ware\hugging.inc
#include tf2ware\redfloor.inc

#include mw_tf2ware2_features.inc
...
StartMinigame() {
    if (GetConVarBool(ww_enable) && g_enabled && (status == 0) && (GetTeamClientCount(2) >= 1) && (GetTeamClientCount(3) >= 1) && g_waiting == false) {
        ...
        if (GetConVarBool(ww_music)) EmitSoundToAll(MUSIC_START, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL,GetSoundMultiplier());
...
public Action:Speedup_timer(Handle:hTimer) {
    if (status == 3) {
        if ((GetConVarInt(ww_speed) >= 4)  && (bossBattle == false)) {
            ...
            if (GetConVarBool(ww_music)) EmitSoundToAll(MUSIC_BOSS, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL,GetSoundMultiplier());
            for (new i = 1; i <= MaxClients; i++) {
                if (IsValidClient(i) && (!(IsFakeClient(i)))) {
        ...
        if ((GetConVarInt(ww_speed) < 4) && (bossBattle == false)) {
            if (GetConVarBool(ww_music)) EmitSoundToAll(MUSIC_SPEEDUP, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL,GetSoundMultiplier());
            for (new i = 1; i <= MaxClients; i++) {
                if (IsValidClient(i) && (!(IsFakeClient(i)))) {
...
public Action:Victory_timer(Handle:hTimer) {
    if ((status == 4) && (bossBattle)) {
        ...
        if (GetConVarBool(ww_music)) EmitSoundToAll(MUSIC_GAMEOVER, SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL,GetSoundMultiplier());
